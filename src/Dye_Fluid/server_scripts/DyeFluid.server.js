if (checkConfigSwitch('dye-fluid')) {
  ...
  ServerEvents.recipes(event => {
    ...
    for (let colorName of Object.keys(DyeFluid.colorTable)) {
      create.mixing( // dye[item] + water[fluid] = dye[fluid]
      ...
      create.filling( // dye[fluid] + dye_base[item] = dye[item]
        [`minecraft:${colorName}_dye`],
        ['kubejs:dye_base', DyeFluid.getFluid(colorName)]
      )
      if (colorName in DyeFluid.mixingTable) { // dye[fluid] + dye[fluid] + ... = dye[fluid]
        for (let ingredient of DyeFluid.mixingTable[colorName].values()) {
          let inputs = [], totalCount = ingredient.length
    ...
    minecraft.crafting_shapeless(
      'kubejs:dye_base',
      ['minecraft:string', '#minecraft:wool']
    )
  })
}